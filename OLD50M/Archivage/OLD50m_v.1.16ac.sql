...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        SELECT 'Mairie' AS nature,                                    -- Attribut nature
			...
			ST_Force2D(                                            -- Force la géométrie en 2D
              ST_Intersection(                                     -- Intersecte les zones d’activité et les bâtiments
                ST_Union(t.geometrie),                             -- Union des géométries des zones d’activité
                ST_Union(b.geometrie)                              -- Union des géométries des bâtiments
              )
            ) AS geom
        FROM r_bdtopo.zone_d_activite_ou_d_interet t               -- Source : zones d’activités et d'intérêts issues de la BDTopo
        JOIN r_bdtopo.batiment b ON ST_Intersects(t.geometrie, b.geometrie)  -- Jointure spatiale bâtiments/zones
        JOIN r_cadastre.geo_commune c ON ST_Intersects(b.geometrie, c.geom) -- Jointure spatiale avec la commune
        WHERE t.nature = 'Mairie'                                  -- Filtrage : nature = Mairie
        AND c.idu = 'xxx'                                          -- Commune cible
        GROUP BY t.nature, t.geometrie, idu                        -- Agrégation pour ST_Union
        ...
        SELECT 'Eglise' AS nature,                                    -- Attribut nature
            ...
            'DU_26xxx' AS partition,                               -- Attribut de partition générique
            c.idu,                                                  -- Code INSEE
			ST_Force2D(ST_Intersection(b.geometrie, c.geom)) AS geom-- Intersection bâtiment/commune
        FROM r_bdtopo.batiment b                                   -- Source : bâtiments issus de la BDTopo
        JOIN r_cadastre.geo_commune c ON ST_Intersects(b.geometrie, c.geom)  -- Jointure spatiale
