...
BEGIN
     ...
     IF NOT EXISTS (
        ...
        SELECT 'Mairie' AS nature,                                 -- Nature du bâtiments
               ...
               ST_Force2D(                                         -- Force la géométrie en 2D
                  ST_Intersection(                                 -- Intersecte les zones d’activité et les bâtiments
                     ST_Union(t.geom),                        -- Union des géométries des zones d’activité en une seule entités par attributs
                     ST_Union(b.geom))                        -- Union des géométries des bâtiments en une seule entités par attributs
               ) AS geom
		...
		ON ST_Intersects(t.geom, b.geom)                 -- Condition : Jointure spatiale bâtiments/zones
        JOIN r_cadastre.geo_commune c 
		ON ST_Intersects(b.geom, c.geom)                      -- Condition : Jointure spatiale avec la commune
        WHERE t.nature = 'Mairie'                                  -- Filtre : nature est égale Mairie
        AND c.idu = 'xxx'                                          -- Filtre : Commune cible
        GROUP BY t.nature, t.geom, idu                        -- Regroupe par attributs
        ...
        SELECT 'Eglise' AS nature,                                 -- Nature du bâtiments
               ...
               c.idu,                                              -- Code INSEE
               ST_Force2D(
                  ST_Intersection(b.geom, c.geom)) AS geom    -- Intersecte les bâtiment et la commune
		...
		ON ST_Intersects(b.geom, c.geom)                      -- Condition : Jointure spatiale
