...
SELECT c.geo_commune,                                 			-- Code INSEE commune (identifiant)
       ST_Buffer(c.geom, 100)::GEOMETRY(MULTIPOLYGON, 2154) AS geom                -- Buffer 100m autour du contour communal
...
GROUP BY comptecommunal,idu;                                                  -- Regroupement par compte communal et par parcelle
...
--                 Les anneaux intérieurs sont supprimés pour affecter leur surface au polygone considéré
--                 comme si l'arbitrage avait été réalisé, mais ils sont conservés s'ils correspondent
--                 à des surfaces déjà arbitrées pour d'autres propriétaires.
...
--                 - Projection en Lambert 93 (SRID 2154)
-- CTE poly_simples
--                 - décomposition Multipolygone en Polygone (ST_Dump)
-- CTE rings_poly_simples
--                 - décomposition Polygone en anneaux (ST_DumpRings)
-- CTE anneaux_interieurs_a_conserver
--                 - filtrage des anneaux intérieurs à conserver (ST_ExteriorRing + ST_Intersects)
-- CTE anneaux_exterieurs
--                 - selection de tous les anneaux extérieurs (ST_ExteriorRing + ring_index = 0)
-- CTE assemblage_anneaux
--                 - regroupement des anneaux extérieurs et des anneaux intérieurs conservés (UNION ALL)
-- CTE reconstruction_pg
--                 - reconstruction des polygones à partir des lignes des anneaux conservés (ST_MakePolygon)
-- CTE reconstruction_pg
--                 - reconstruction des polygones à partir des lignes des anneaux conservés (ST_MakePolygon)

--                 - reconstruction des Multipolygones (ST_Multi + ST_CollectionExtract + ST_Union)
...
-- Etape pour supprimer les anneaux interieurs et de facto affecter ces surfaces au polygone considéré
-- comme si l'arbitrage avait été réalisé
-- mais sans supprimer les anneaux intérieurs s'ils correspondent à des surfaces déjà arbitrées pour d'autres propriétaires.
-- Les anneaux intérieurs seront conservés s'ils intersectent des surfaces déjà arbitrées.

-- Décomposition des MultiPolygons en polygones simples
poly_simples AS (
	 SELECT d.comptecommunal,                                   -- Compte communal
		    (ST_Dump(d.geom)).path AS path1,                    -- Identifiant du polygone dans le multipolygone
		    (ST_Dump(d.geom)).geom AS geom                      -- Géométrie du polygone simple
	FROM decoupe d
),
...
-- Filtrage des anneaux intérieurs qui intersectent des polygones attribués à d'autres propriétaires
-- et qui sont donc déjà arbitrés au profit d'un autre propriétaire (il faut les conserver)
anneaux_interieurs_a_conserver AS (
	 SELECT DISTINCT
			rps.comptecommunal,                                  -- Compte communal
	 ...
	 LEFT JOIN decoupe d
	 ON rps.comptecommunal != d.comptecommunal
	 WHERE ST_Intersects(rps.geom, d.geom)
	 AND  rps.ring_index > 0                                     -- filtre pour travailler sur les anneaux intérieurs
),
anneaux_exterieurs AS (
	 SELECT DISTINCT
			rps.comptecommunal,                                  -- Compte communal
		    rps.path1,                                           -- Identifiant du polygone
		    rps.ring_index,                                      -- Index de l’anneau
		    ST_ExteriorRing(rps.geom) AS geom                    -- Conversion des anneaux en lignes
	 FROM rings_poly_simples rps
	 WHERE rps.ring_index = 0                                    -- filtre pour travailler sur les anneaux extérieurs
),
assemblage_anneaux AS (
	 SELECT DISTINCT
			aic.comptecommunal,                                  -- Compte communal
		    aic.path1,                                           -- Identifiant du polygone
		    aic.ring_index,                                      -- Index de l’anneau
		    aic.geom                    
	 FROM anneaux_interieurs_a_conserver aic

	 UNION ALL
	 
	 SELECT DISTINCT
			ae.comptecommunal,                                  -- Compte communal
		    ae.path1,                                           -- Identifiant du polygone
		    ae.ring_index,                                      -- Index de l’anneau
		    ae.geom                    
	 FROM anneaux_exterieurs ae
),
...
reconstruction_pg AS (
	 ...
	 FROM assemblage_anneaux ma                                      -- Source : assemblage des anneaux intérieurs et extérieurs
...
---- - Supprimer les zones non cadastrées de ces surfaces                                                    ----
...
--
---- Méthode :
--                 - Différence géométrique entre les regroupements des tampons hors zone U et des zones OLD
...
--
---- Méthode :
--                 - Intersection spatiale préalable pour accélérer le calcul (ST_Intersects)
...
---- Création de la table "26xxx_trou4" : 
---- Simplification de l’enveloppe des polygones de la couche 26xxx_trou3 pour réduire le nombre de sommets
--  et préparer le calcul des lignes de visée qui déterminera les parcelles baties directement visibles

---- Description : Cette table simplifie les géométries polygones issues de "26xxx_trou3", 
--                 en préservant la topologie.
--
---- Méthode :
--                 - Simplification des polygones (ST_SimplifyPreserveTopology)
--
---- Attributs :
--                 -> id     : identifiant structuré du polygone au sein de la géométrie d’origine
--                 -> geom     : géométrie simplifiée Polygon, SRID 2154 (Lambert 93)

DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_trou4";                     -- Supprime la table si elle existe déjà
COMMIT; 

CREATE TABLE "26xxx_wold50m"."26xxx_trou4" AS                           -- Crée une nouvelle table des enveloppes des trous individualisés
SELECT
	path AS id,
	ST_SimplifyPreserveTopology(
		ST_Buffer(geom, 0.5)
		,1) AS geom
FROM "26xxx_wold50m"."26xxx_trou3";
COMMIT;
...
---- Création de la table "26xxx_trou5" : 
---- Ajout d'une liste de compte communaux correspondants aux parcelles baties en visibilité directe avec le trou
--   sans les parcelles bâties dont les bâtiments sont éloignés de plus de 50 mètres du trou
...
--
---- Méthode :
--                 - premier tri des comptes communaux concernés par l'arbitrage dans chaque trou
--                 - central_pts :extraction des sommets de l'enveloppe simplifiée de chaque trou
--                 - b_polys : sélection des géométries simplifiées des parcelles baties à arbitrer pour chaque trou
--                 - b_pts : extraction des sommets de ces géométries
--                 - lignes_visee : constructions des lignes de visée entre les sommets des trous et les parcelles baties (distance < 50 m)
--                 - lignes_visee_directes : Déterminer les lignes de visées atteignant une parcelle batie A non bloquées par d'autres parcelles baties concernées par le même trou
--                 - ncc_parcelle_visible : Déduire les comptes communaux des parcelles visibles pour chaque trou
--                 - liste_ncc_visible : agréger la liste des comptes communaux visibles
--
---- Attributs :
--                 -> idtrou        : identifiant du trou
--                 -> nb_liste1     : nombre de comptes communaux de la liste élaborée lors du premier tri
--                 -> nb_liste2     : nombre de compte communaux visibles
--                 -> liste_ncc_1   : liste des comptes communaux de la liste élaborée lors du premier tri
--                 -> liste_ncc_2   : liste des comptes communaux visibles
--                 -> geom          : géométrie du trou Polygon, SRID 2154 (Lambert 93)

DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_trou5";                     -- Supprime la table si elle existe déjà
COMMIT; 

CREATE TABLE "26xxx_wold50m"."26xxx_trou5" AS                           -- Crée une nouvelle table des trous individualisés
WITH
premier_tri AS (
	SELECT tr3.path AS idtrou,                                                    -- identifiant unique
	   ARRAY_AGG(DISTINCT t.comptecomm1 ORDER BY t.comptecomm1) AS liste_ncc_1,   -- Liste triée et sans doublons
       tr3.geom                                                                   -- Géométrie résultante (Lambert 93)
	FROM "26xxx_wold50m"."26xxx_trou3" tr3
	INNER JOIN "26xxx_wold50m"."26xxx_tampon_ihu" t                               -- Source : zones tamponnées par compte communal
	ON (ST_Overlaps(tr3.geom, ST_Buffer(t.geom, -.01))                            -- Test d’appartenance spatiale Overlaps
		OR ST_Contains(t.geom, tr3.geom))                                         -- Test d’appartenance spatiale si aucune des deux géométrie ne recouvre l'autre
	WHERE GeometryType(tr3.geom) IN ('POLYGON', 'MULTIPOLYGON')                   -- Filtre : types polygonaux uniquement
	GROUP BY tr3.path, tr3.geom                                                   -- Agrégation par identifiant et géométrie
	),
---- Étape 1 - Extraire tous les sommets (points) des enveloppes simplifiées des polygones
-- Sommets du contour de l'enveloppe du trou (trou5, polygone central)
central_pts AS (
    SELECT
        id AS idtrou,
		(ST_DumpPoints(
			tr4.geom)).geom AS pt
    FROM "26xxx_wold50m"."26xxx_trou4" tr4
),
-- Sommets des autres polygones (parcelles bâties avec numéro de compte communal figurant dans la liste_ncc1 du trou)
b_polys AS (
    SELECT p.comptecommunal, p.idu, premier_tri.idtrou, 
	 ST_SetSRID(                                                  -- Définit la projection EPSG:2154
		ST_CoverageSimplify(p.geom, 2) OVER ()                    -- geométrie simplifiée pour réduire le nombre de sommets
		,2154) AS covgeom,                                        -- Géométrie résultante (Lambert 93)
		p.geom
    FROM "26xxx_wold50m"."26xxx_parcelle_batie" p
	INNER JOIN premier_tri
	ON p.comptecommunal = ANY(premier_tri.liste_ncc_1)            -- Jointure externe sur les comptes communaux
),
b_pts AS (
    SELECT
        b.idtrou,
...
---- Étape 2 — Créer toutes les lignes de visée (sommet A → sommet B)
lignes_visee AS (
    SELECT DISTINCT
		b.idtrou,
		b.bid,
...
---- Étape 3 — Déterminer les lignes de visées atteignant un polygone A non bloquées par les autres polygones du trou concerné
lignes_visee_directes AS (
	SELECT  lv.idtrou, lv.bid, lv.comptecommunal, lv.line_geom AS geom
    FROM lignes_visee lv
	-- conditions d'intersection avec un autre polygone
    WHERE NOT EXISTS (
        SELECT 1
        FROM b_polys bp
        WHERE bp.idtrou = lv.idtrou
        AND bp.idu != lv.bid
        AND ST_Crosses(lv.line_geom, bp.geom)      -- importance de ST_Crosses pour garder les parcelles avec un seul segment en vis à vis de l'ilot
	)                                              -- ST_Intersects renverrait cette parcelle comme masquée
),
---- Étape 4 — Déduire les comptes communaux des parcelles visibles pour chaque trou
ncc_parcelle_visible AS (
	SELECT lvd.idtrou, lvd.comptecommunal
    FROM lignes_visee_directes lvd
	GROUP BY lvd.comptecommunal, lvd.idtrou   --, lvd.bid   -- possibilité de lister les parcelles visibles par chaque trou
),
---- Étape 5 — agréger la liste des comptes communaux visibles
liste_ncc_visible AS (
	SELECT 
	idtrou,                                                                 -- identifiant du trou
	array_agg(comptecommunal ORDER BY comptecommunal) AS liste_trou_ncc_vis -- compte communal des parcelles visibles
...
CREATE INDEX idx_26xxx_trou5_geom                                
ON "26xxx_wold50m"."26xxx_trou5"
USING gist (geom);
COMMIT;

--*-----------------------------------------------------------------------------------------------------------*--

---- Création de la table "26xxx_trou6" : 
---- Sélection des trous de la table "26xxx_trou5" n'ayant qu'un seul numéro de compte communal dans la liste
--   liste_trou_ncc_vis, ces trous sont donc déjà attribués, pas besoin de calcul supplémentaire
--   Les entités seront directement intégrées à l'étape 26xxx_result2

---- Description : Cette table sélectionne les entités issues de "26xxx_trou5" qui n'ont qu'un seul numéro 
--                 de compte communal visible.
--
---- Méthode :
--                 - filtre sur nb_liste2
--
---- Attributs :
--                 -> idtrou        : identifiant du trou
--                 -> geom          : géométrie du trou Polygon, SRID 2154 (Lambert 93)

DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_trou6";    -- Supprime la table si elle existe déjà
COMMIT; 

CREATE TABLE "26xxx_wold50m"."26xxx_trou6" AS          -- Crée une nouvelle table des trous individualisés
SELECT
tr5.idtrou,
tr5.liste_trou_ncc_vis[1] AS comptecommunal,
ST_SetSRID(
	ST_Multi(
		ST_MakeValid(tr5.geom)                         -- Corrige les géométries invalides
		),               
	2154) AS geom                                      -- Nomme la colonne résultante
FROM "26xxx_wold50m"."26xxx_trou5" tr5
WHERE tr5.nb_liste2 = 1;
COMMIT;

CREATE INDEX idx_26xxx_trou6_geom                                
ON "26xxx_wold50m"."26xxx_trou6"
USING gist (geom);
COMMIT;

--*-----------------------------------------------------------------------------------------------------------*--

---- Création de la table "26xxx_trou_final" : 
---- Sélection des trous de la table "26xxx_trou5" en enlevant les trous de la table "26xxx_trou6" déjà arbitrés

---- Description : L’objectif est de sélectionner les trou avec un arbitrage à réaliser
--
---- Méthode :
--                 - requête de non correspondance WHERE .... IS NULL
--
---- Attributs :
--                 -> idtrou        : identifiant du trou
--                 -> nb_liste1     : nombre de comptes communaux de la liste élaborée lors du premier tri
...
DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_trou_final";                     -- Supprime la table si elle existe déjà
COMMIT; 

CREATE TABLE "26xxx_wold50m"."26xxx_trou_final" AS                           -- Crée une nouvelle table des trous individualisés
SELECT
tr5.idtrou,
tr5.nb_liste1,
tr5.nb_liste2,
...
ST_SetSRID(
	...
	2154) AS geom                                  -- Nomme la colonne résultante
FROM "26xxx_wold50m"."26xxx_trou5" tr5
LEFT JOIN "26xxx_wold50m"."26xxx_trou6" tr6
ON tr5.idtrou = tr6.idtrou
WHERE tr6.idtrou IS NULL;
COMMIT;

CREATE INDEX idx_26xxx_trou_final_geom                                
ON "26xxx_wold50m"."26xxx_trou_final"
USING gist (geom);
COMMIT;
...
--*-----------------------------------------------------------------------------------------------------------*--
---- MÉTHODOLOGIE GLOBALE :                                                                                  ----
---- - **Intersection ** entre les tampons des zones de superposition (`tampon_ihu`) et les **trous          ----
----    géométriques restants** (`trou_final`) pour découper les tampons_ihu par les trous final             ----
----    situations de chevauchement foncier.                                                                 ----
---- - ** Récupération des intersections manquantes ** et affectation directe du numero de compte communal   ----
...
---- - **Regroupement multi-comptes** : regroupement des polygones par liste identique de comptes communaux  ----
---- - **Calcul d'une géométrie simplifiée** : pour ces multi polygones en conservant l'identifiant          ----
---- - **Calcul des parcelles visibles par chaque ilot** : les parcelles baties en second plan, masquée par  ----
----     une parcelle visible ne sont pas retenues                                                           ----
---- - ** Récupération des ilots ** qui n'ont qu'une seule parcelle visible et affectation du numero de      ----
----      compte communal de cette parcelle                                                                  ----
---- - ** Sélection des autres ilots ** avec liste compte communal et liste parcelles visibles               ----
--*-----------------------------------------------------------------------------------------------------------*--
---- RÉSULTATS ATTENDUS :                                                                                    ----
---- - Un polygone distinct pour chaque îlot résiduel, correspondant à une zone de superposition non couverte----
---- - Pour chaque polygone, la liste complète des comptes communaux impliqués, et la liste des parcelles    ----
----    visibles, sous forme de tableaux                                                                     ----
---- - Deux tables d'ilots (polygones) dont les responsabilites sont connues et qui seront intégrées dans    ---- 
----   la PARTIE XIV                                                                                         ----
--*-----------------------------------------------------------------------------------------------------------*--
---- CHOIX TECHNIQUES JUSTIFIÉS :                                                                            ----
...
SELECT trf.idtrou,
	   t_ihu.comptecomm1,                                              -- N° du compte communal 1
       t_ihu.comptecomm2,                                              -- N° du compte communal 2
       ST_SetSRID(                                                     -- Définit le système de coordonnées EPSG:2154
          ST_CollectionExtract(                                        -- Extrait uniquement les polygones (type 3) => exclusion des tampons_ihu accolés (intersection de type ligne)
             ST_MakeValid(                                             -- Corrige les géométries invalides
                ST_Intersection(                                       -- Calcule l’intersection trous/superpositions
                   ST_MakeValid(trf.geom),                             -- Valide la géométrie des trous individuels
...
JOIN "26xxx_wold50m"."26xxx_trou_final" AS trf                         -- Source : trous individuels corrigés
ON t_ihu.comptecomm1 = ANY(trf.liste_trou_ncc_vis)                     -- Filtre sur la liste des comptes communaux visibles de la table trou_final          
AND t_ihu.comptecomm2 = ANY(trf.liste_trou_ncc_vis)                    -- Filtre sur la liste des comptes communaux visibles de la table trou_final                
AND ST_Intersects(t_ihu.geom, trf.geom);                               -- Filtre spatial : uniquement les intersections réelles                 
...
---- Création de la table "26xxx_ilot_du_trou_t1_abs" : Sélection des ilôts du trou absents de "26xxx_ilot_du_trou_t1" 
---- Description : Cette table sélectionne les ilôts du trou absents de "26xxx_ilot_du_trou_t1". Ils sont absent car 
--                 sur cette zone il n'y a qu'une seule géométrie de "26xxx_tampon_ihu" avec soit comptecomm1 soit comptecomm2 
--                 qui correspond avec les numéros de compte comptecommunal présent dans la liste_trou_ncc_vis
--                 de la table "26xxx_trou_final"
--                 Il faut donc créer cette géométrie et retenir le comptecomm1 ou comptecomm2 présent dans cette liste,
--                 et ces ilots absents seront alors arbitrés puis intégrés dans "26xxx_result2".
--
---- Méthode :
--                 - Intersection spatiale entre l'union des ilôts du trou t1 et trous individuels
--                 - Nettoyage, récupération du compte communal à attribuer
--                 - Définition du système de coordonnées (EPSG:2154 - Lambert 93)
--
---- Attributs :
--                 -> idtrou         : identifiant du trou
...
DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_ilot_du_trou_t1_abs";
COMMIT;

CREATE TABLE "26xxx_wold50m"."26xxx_ilot_du_trou_t1_abs" AS
WITH
union_ilt1 AS (
	SELECT
	ilt1.idtrou,
	ST_Union(ilt1.geom) AS geom
	FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t1" ilt1
	GROUP BY idtrou
),
difference AS (
	SELECT
	trf.idtrou,
       ST_SetSRID(                                                     -- Définit le système de coordonnées EPSG:2154
          ST_CollectionExtract(                                        -- Extrait uniquement les polygones (type 3) 
             ST_MakeValid(                                             -- Corrige les géométries invalides
                ST_Difference(                                         -- Calcule la différence trous/ union des ilots t1
                   ST_MakeValid(trf.geom),                             -- Valide la géométrie des trous individuels
                   ST_MakeValid(union_ilt1.geom))),                    -- Valide la géométrie de l'union des ilots du trou t1
             3),                                                       -- Type 3 = POLYGON / MULTIPOLYGON
       2154) AS geom                                                   -- Géométrie finale Lambert 93
	FROM union_ilt1                        
	JOIN "26xxx_wold50m"."26xxx_trou_final" AS trf                       
	ON union_ilt1.idtrou = trf.idtrou                                  -- Filtre avec correspondance sur l'identifiant idtrou          
),
dumpdiff AS (
	SELECT
	difference.idtrou,
...
nettoyage AS (
	...
	FROM dumpdiff
	WHERE surface > 0.1
	GROUP BY dumpdiff.idtrou                                           -- Filtre de surface pour enlever les artefacts
),
jointure1 AS (
	SELECT 
	nettoyage.idtrou,
	...
	nettoyage.geom
	FROM nettoyage
	JOIN "26xxx_wold50m"."26xxx_tampon_ihu" AS t_ihu                   -- Source : zones de superposition (tampons IHU)
	ON ST_Within(
		ST_PointOnSurface(nettoyage.geom),
		t_ihu.geom)                                                    -- Filtre spatial : uniquement les intersections réelles                 
),
jointure2 AS (
	SELECT 
	jointure1.idtrou,
	...
	ON ST_Within(
		...
		trf.geom)                                 -- Filtre spatial : uniquement les intersections réelles             
)
SELECT 
jointure2.idtrou,
...
ST_SetSRID(
	ST_Multi(
		ST_MakeValid(jointure2.geom)              -- Corrige les géométries invalides
		),               
	2154) AS geom                                 -- Nomme la colonne résultante
FROM jointure2
...
DELETE FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t1_abs"            
WHERE geom IS NULL 
OR ST_IsEmpty(geom) 
OR NOT ST_IsValid(geom);

CREATE INDEX idx_26xxx_ilot_du_trou_t1_abs_geom                              
ON "26xxx_wold50m"."26xxx_ilot_du_trou_t1_abs"
USING gist (geom);
COMMIT;
...
--
---- Méthode :
--                 - Extraction des contours (ST_DumpSegments)
...
-------------------------------------------------------------------------------------
-- Code modifié nov 2025 pour réduire la complexité du calcul et éviter l'erreur bad_alloc
...
collection_poly AS (
	...
	FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t2" iltt2
),
-- décomposition en segments
segments AS (                               -- type linestring avec deux sommets, avec plusieurs entités
	SELECT DISTINCT
	(ST_DumpSegments(
...
-- regroupement en collection
collection_segment AS (                     -- type multilinestring, avec une seule entité
	SELECT 
	ST_Collect(segments.geom) AS geom
	FROM segments
),
-- simplification avec ST_UnaryUnion
unaryunion AS (                                   -- type multilinestring, avec une seule entité
	SELECT 
	ST_UnaryUnion(
		collection_segment.geom) AS geom
	FROM collection_segment
),
-- polygonisation à partir de l'objet multilinstring
polygonize AS (
	SELECT
	ST_collectionExtract(                   -- type multipolygone, avec une entité
		ST_Polygonize(unaryunion.geom)
	,3) AS geom
	FROM unaryunion
)
-- extraction des polygones simples
SELECT
	(ST_Dump(polygonize.geom)).path AS id,
	(ST_Dump(polygonize.geom)).geom AS geom        -- type polygone, avec plusieurs entités
FROM polygonize;
COMMIT;

ALTER TABLE "26xxx_wold50m"."26xxx_ilot_du_trou_t3"
ALTER COLUMN geom 
TYPE geometry(MultiPolygon, 2154)
USING ST_SetSRID(geom, 2154);   
...
SELECT t3.id, --row_number() OVER () as id,                                     -- Crée un identifiant unique
	   array_length(ARRAY_AGG(DISTINCT t.comptecomm1), 1) AS nb_liste_ncc,
	   ARRAY_AGG(DISTINCT t.comptecomm1 ORDER BY t.comptecomm1) AS liste_ncc,   -- Liste triée et sans doublons
       ST_SetSRID(                                                              -- Définit la projection EPSG:2154
...
FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t3" t3
INNER JOIN "26xxx_wold50m"."26xxx_tampon_ihu" t
ON ST_Within(ST_PointOnSurface(t3.geom), t.geom)
INNER JOIN "26xxx_wold50m"."26xxx_trou5" tr5
ON ST_Intersects(t3.geom, tr5.geom)
AND t.comptecomm1 = ANY(tr5.liste_trou_ncc_vis)
--WHERE GeometryType(t3.geom) IN ('POLYGON', 'MULTIPOLYGON')
...
---- Création de la table "26xxx_ilot_du_trou_t5" : Résultat final des zones de superpositions non couvertes ayant plusieurs comptes communaux.
...
DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_ilot_du_trou_t5";
COMMIT;

CREATE TABLE "26xxx_wold50m"."26xxx_ilot_du_trou_t5" ( 
...
INSERT INTO "26xxx_wold50m"."26xxx_ilot_du_trou_t5" (liste_ncc, geom)
...
WHERE ST_Area(it4.geom) > 0.001                                        -- Filtre de surface pour enlever des artefacts
GROUP BY it4.liste_ncc;                                                -- Regroupement par combinaison de comptes

DELETE FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t5"
WHERE geom IS NULL 
OR ST_IsEmpty(geom)
OR NOT ST_IsValid(geom)
OR liste_ncc IS NULL
...
CREATE INDEX idx_26xxx_ilot_du_trou_t5_geom
ON "26xxx_wold50m"."26xxx_ilot_du_trou_t5"
USING gist (geom);
COMMIT;

--*-----------------------------------------------------------------------------------------------------------*--

---- Création de la table "26xxx_ilot_du_trou_t6" : 
---- Nettoyage de l'enveloppe de la couche 26xxx_ilot_du_trou_t5 pour réduire son nombre de sommets
--  en vue du calcul des lignes de visée

---- Description : Cette table crée les géométries simplifiées des polygones des ilots en préservant la topologie de éventuels anneaux
--                 L'utilisation de cette géométrie simplifiée permettra de réduire le nombre de lignes de visées ci-après
--
---- Méthode :     Tampon de 50 cm pour effacer les crénelages ou peignes (ST_Buffer)
--                 Calcul d'une géométrie simplifiée, avec moins de sommets (ST_SimplifyPreserveTopology)
--
---- Attributs :
--                 -> id : identifiant unique de l'ilot du trou
--                 -> nb_liste_ncc  : nombre de comptes communaux présents dans la liste liste_ncc
--                 -> geom       : géométrie MultiPolygon (EPSG:2154 - Lambert 93)

DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_ilot_du_trou_t6";                     -- Supprime la table si elle existe déjà
COMMIT; 

CREATE TABLE "26xxx_wold50m"."26xxx_ilot_du_trou_t6" AS                           -- Crée une nouvelle table des enveloppes des trous individualisés
    SELECT
		id,
		liste_ncc,
 		 ST_SimplifyPreserveTopology(
			ST_Buffer(geom, 0.5)
			,1) AS geom
   FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t5";
COMMIT;

CREATE INDEX idx_26xxx_ilot_du_trou_t6_geom                                
ON "26xxx_wold50m"."26xxx_ilot_du_trou_t6"
USING gist (geom);
COMMIT;

--*-----------------------------------------------------------------------------------------------------------*--

---- Création de la table "26xxx_ilot_du_trou_t7" : calcul des parcelles visibles par l'ilot
---- Description : Cette table stocke les ilots associés à plusieurs comptes communaux et 
--                 les parcelles baties en visibilité directe de chacun de ces ilots.
--                 Les comptes et les parcelles visibles sont regroupés dans 2 listes (tableau) avec leurs géométries fusionnées
--
---- Méthode :
--                 - Création de lignes entre les sommets de la géométrie simplifiée de l'ilot et les sommets des parcelles baties
--                 - Vérification si ces lignes survolent une autre parcelle batie
--                 - Sélection par ilot des parcelles baties qui ont au moins une ligne de visée directe
--
---- Attributs :
--                 -> idilotrou : identifiant unique de l'ilot du trou
--                 -> nb_liste_ncc  : nombre de comptes communaux présents dans la liste liste_ncc
--                 -> nb_liste_parc_visible  : nombre de parcelles visibles présentes dans la liste liste_parc_vis
--                 -> liste_ncc  : liste des comptes communaux concernés par l'arbitrage
--                 -> liste_ilot_parc_vis  : liste des parcelles baties en visibilité directe de l'ilot
--                 -> geom       : géométrie MultiPolygon (EPSG:2154 - Lambert 93)

DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_ilot_du_trou_t7";
COMMIT;

CREATE TABLE "26xxx_wold50m"."26xxx_ilot_du_trou_t7" AS
WITH
---- Étape 1 - Extraire tous les sommets (points) des enveloppes des trous
-- Sommets du contour de l'enveloppe du trou (trou5, polygone central)
central_pts AS (
    SELECT
        id AS idilotrou,
		...
		(ST_DumpPoints(
			it6.geom)).geom AS pt
    FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t6" it6
),
-- Sommets des autres polygones (parcelles bâties avec numéro de compte communal figurant dans la liste_ncc1 du trou)
b_polys AS (
	...
	ON p.comptecommunal = ANY(central_pts.liste_ncc)              -- Jointure externe sur les comptes communaux
),
b_pts AS (
    SELECT
        b.idilotrou,
...
---- Étape 4 — Déduire les identifiants des parcelles visibles pour chaque trou
parcelle_visible_du_trou AS (
	SELECT lvd.idilotrou, lvd.bid 
    FROM lignes_visee_directes lvd
	GROUP BY lvd.bid, lvd.idilotrou  
),
---- Étape 5 — agréger la liste des parcelles visibles
liste_parc_visible AS (
	SELECT 
	idilotrou, -- identifiant du trou
	array_agg(bid ORDER BY bid) AS liste_parc_vis -- idu des parcelles visibles
...
lpv.liste_parc_vis,
ilt5.geom
FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t5" ilt5
JOIN liste_parc_visible lpv
ON ilt5.id = lpv.idilotrou;
COMMIT;

CREATE INDEX idx_26xxx_ilot_du_trou_t7_geom                                
ON "26xxx_wold50m"."26xxx_ilot_du_trou_t7"
USING gist (geom);
COMMIT;

--*-----------------------------------------------------------------------------------------------------------*--

---- Création de la table "26xxx_ilot_du_trou_t8" : extraction de la table 26xxx_ilot_du_trou_t7 
---- Description : Cette table stocke les ilots associés à plusieurs comptes communaux et 
--                 avec une seule parcelle batie en visibilité directe de chacun de ces ilots.
--                 Ces ilots seront alors arbitrés avec le compte communal de la seule parcelle visible puis intégrés dans "26xxx_result2".
--
---- Méthode :
--                 - Filtrage selon le nombre de valeurs du tableau de parcelles visibles égal à 1
--                 - Attribution du compte communal de la seule parcelle batie visible
----
---- Attributs :
--                 -> idilotrou : identifiant unique de l'ilot du trou
--                 -> comptecommunal  : compte communal correspondant à la seule parcelle visible
--                 -> geom       : géométrie MultiPolygon (EPSG:2154 - Lambert 93)

DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_ilot_du_trou_t8";
COMMIT;

CREATE TABLE "26xxx_wold50m"."26xxx_ilot_du_trou_t8" AS
WITH
filtrage AS (
	SELECT
	ilt7.idilotrou,
	...
	ilt7.liste_parc_vis,
	ilt7.geom
	FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t7" ilt7
...
SELECT 
	...
	filtrage.geom
FROM filtrage
LEFT JOIN "26xxx_wold50m"."26xxx_parcelle_batie" pbat
ON filtrage.liste_parc_vis[1] = pbat.idu;
COMMIT;

CREATE INDEX idx_26xxx_ilot_du_trou_t8_geom                                
ON "26xxx_wold50m"."26xxx_ilot_du_trou_t8"
USING gist (geom);
COMMIT;

--*-----------------------------------------------------------------------------------------------------------*--

---- Création de la table "26xxx_ilot_final" : extraction de la table 26xxx_ilot_du_trou_t7
---- Description : Cette table stocke les zones de superpositions non couvertes associées à plusieurs comptes communaux et 
--                 à plusieurs parcelles baties en visibilité directe de l'ilot.
--                 Les comptes et les parcelles visibles figurent dans 2 listes (tableau) avec leurs géométries fusionnées
--
---- Méthode :
--                 - Filtrage selon le nombre de valeurs du tableau de parcelles visibles supérieur à 1
--
---- Attributs :
--                 -> idilotrou : identifiant unique de l'ilot du trou
--                 -> nb_liste_ncc  : nombre de comptes communaux présents dans la liste liste_ncc
...
DROP TABLE IF EXISTS "26xxx_wold50m"."26xxx_ilot_final";
COMMIT;

CREATE TABLE "26xxx_wold50m"."26xxx_ilot_final" AS
	SELECT
	ilt7.idilotrou,
	ilt7.nb_liste_ncc,
	ilt7.nb_liste_parc_visible,
...
CREATE INDEX idx_26xxx_ilot_final_geom                                
ON "26xxx_wold50m"."26xxx_ilot_final"
USING gist (geom);
COMMIT;
...
--                 de superpositions non couvertes. Chaque polygone est finalement attribué à un compte communal 
--
---- Méthode :     
...
WITH
jointure1 AS (          -- sélection des parcelles (idu, geom) visibles
	SELECT
	ilfi.idilotrou,
	p.comptecommunal,
	p.idu,
	...
	FROM "26xxx_wold50m"."26xxx_ilot_final" ilfi
	INNER JOIN "26xxx_wold50m"."26xxx_parcelle_batie" p
	ON p.idu = ANY(ilfi.liste_parc_vis)
	ORDER BY ilfi.idilotrou, p.comptecommunal
),
jointure2 AS (           -- sélection des parcelles visibles avec leurs points interpolés
	SELECT
	jt1.idilotrou,
	jt1.comptecommunal,
	jt1.idu,
	...
	FROM jointure1 jt1
	LEFT JOIN "26xxx_wold50m"."26xxx_pt_interpol" pi
	ON pi.idu = jt1.idu
	ORDER BY jt1.idilotrou, jt1.comptecommunal, jt1.idu
),
regroup1 AS(           -- regroupement des points interpolés par ilot du trou et compte communal
	SELECT
	jt2.idilotrou,
	jt2.comptecommunal,
	jt2.ilotrou_geom,
...
voro1 AS (
SELECT 
    regroup1.idilotrou,                               -- Récupère l’identifiant unique de l’entité "ilfi"
	   ...
	   ST_MakeValid(
        (ST_DUMP(                                     -- Décompose les géométries complexes en éléments individuels (ici pour Voronoï)
          ST_VoronoiPolygons(                         -- Génère les polygones de Voronoï à partir d’un ensemble de points
            ST_Collect(regroup1.geom),                       -- Agrège toutes les géométries de la table/alias "p" en une seule géométrie
            0,                                        -- Tolérance = 0 → calcule des polygones Voronoï exacts sans simplification
            ST_Envelope(regroup1.ilotrou_geom)))                  -- Limite l’extension des polygones Voronoï à l’emprise de la géométrie "ilfi"
        ).geom                                        -- Récupère la géométrie extraite par ST_DUMP
      ))
    ,2154) AS geom                                    -- Applique le SRID et renomme le résultat en "geom"
FROM regroup1                                         -- Source : îlots de trous avec cc
WHERE regroup1.geom IS NOT NULL                       -- Filtre : Exclut les géométries invalides
--AND regroup1.idilotrou = 645
GROUP BY idilotrou,	regroup1.ilotrou_geom             -- Regroupe par ilot
)
SELECT DISTINCT
voro1.idilotrou,
pi.comptecommunal,
voro1.geom
FROM voro1
INNER JOIN "26xxx_wold50m"."26xxx_ilot_final" ilfi
ON voro1.idilotrou = ilfi.idilotrou
INNER JOIN "26xxx_wold50m"."26xxx_pt_interpol" pi
...
---- Création de la table "26xxx_ilot_voronoi_t2" : Regroupement des polygones Voronoï des zones de superpositions non couvertes par compte communal.
...
--
---- Attributs :
--                 -> idilotrou               : identifiant de l’îlot
...
SELECT iv2.idilotrou,                                                                -- Identifiant de l'îlot d'origine
       ...
       ST_SetSRID(                                                            -- Définit le système de projection EPSG:2154
          ST_CollectionExtract(                                               -- Extrait uniquement les polygones (type 3)
             ...
             3),
        2154) AS geom                                                         -- Nomme la colonne résultante
FROM "26xxx_wold50m"."26xxx_ilot_voronoi_t1" iv2                              -- Source : cellules Voronoï attribuées
GROUP BY iv2.idilotrou, iv2.comptecommunal;                                          -- Regroupe par îlot et compte communal
...
---- Création de la table "26xxx_ilot_voronoi_t3" : Découpe des polygones Voronoï avec les zones de superpositions non couvertes.
...
--
---- Attributs :
--                 -> idilotrou               : identifiant de l’îlot d’origine
          ...
          ST_CollectionExtract(                                             -- Extrait uniquement les polygones (type 3)
             ST_MakeValid(                                                  -- Valide après intersection
                ST_Intersection(                                            -- Intersection Voronoï / îlot multi-propriétaire
                   ...
                   ST_MakeValid(iv2.geom))),                                -- Géométrie Voronoï validée          
          3), 
       2154) AS geom                                                        -- Géométrie résultante en MultiPolygon
FROM "26xxx_wold50m"."26xxx_ilot_final" ilfi                               -- Source : îlots de superposition non couverts
INNER JOIN "26xxx_wold50m"."26xxx_ilot_voronoi_t2" iv2                      -- Source : cellules Voronoï regroupées par propriétaire
ON ilfi.idilotrou = iv2.idilotrou                                                         -- Jointure sur l’identifiant d’îlot
AND ST_Intersects(ilfi.geom, iv2.geom);                                     -- Condition spatiale : intersection réelle
...
--                 (compte communal), en fusionnant toutes les géométries issues de `26xxx_ilot_voronoi_t3`. 
...
SELECT iv3.comptecommunal,                                                  -- N° du compte communal (propriétaire)
       ST_SetSRID(                                                          -- Définit le système de projection EPSG:2154
          ST_CollectionExtract(                                             -- Extrait uniquement les polygones (type 3)
             ST_MakeValid(                                                  -- Corrige les géométries invalides
                ST_Union(iv3.geom)),                                        -- Fusionne toutes les géométries du même compte communal       
          3), 
       2154) AS geom                                                        -- Géométrie résultante en Lambert-93
FROM "26xxx_wold50m"."26xxx_ilot_voronoi_t3" iv3                            -- Source : zones Voronoï découpées par propriétaire
GROUP BY iv3.comptecommunal;                                                -- Regroupement par compte communal
...
---- Création de la table "26xxx_result2" : Fusion des zones initiales, des trous préaffectés et des zones Voronoï par propriétaire.
---- Description : Cette table fusionne conditionnellement les géométries des trois tables sources :
--                 "26xxx_result1" (zones initiales), "26xxx_trou7" (trous préaffectés) et "26xxx_ilot_voronoi_rg" (zones Voronoï).
...
WITH 
union_all AS (                                           -- CTE : agrégation des sources
	SELECT 
		r1.comptecommunal,                               -- N° de compte communal
		ST_SetSRID(                                      -- Définit le système de projection EPSG:2154
			ST_CollectionExtract(                        -- Extrait uniquement les polygones (type 3)
	...
	UNION ALL                                            -- Agrège les tables
    
	SELECT 
		comptecommunal,                                  -- N° de compte communal
		geom                                 
	FROM "26xxx_wold50m"."26xxx_trou6"                   -- Source : trous préattribués
	WHERE geom IS NOT NULL                               -- Filtre les géométries NULL
	AND NOT ST_IsEmpty(geom)                             -- Filtre les géométries vides
    ...
    SELECT 
		comptecommunal,                                  -- N° de compte communal
	    geom                             
    FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t1_abs"     -- Source : ilots du trou absents
	...
	SELECT
		comptecommunal,                                  -- N° de compte communal
		geom                                 
	FROM "26xxx_wold50m"."26xxx_ilot_du_trou_t8"         -- Source : ilots de trous préattribués
	WHERE geom IS NOT NULL                               -- Filtre les géométries NULL
	AND NOT ST_IsEmpty(geom)                             -- Filtre les géométries vides
